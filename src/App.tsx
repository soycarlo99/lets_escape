...
// Define types
export type Language = 'javascript' | 'python' | 'java' | 'cpp' | 'csharp' | 'go' | 'rust' | 'typescript';
...
const App: React.FC = () => {
  ...
  // Update editor content when language or puzzle changes
  ...
  const extractFunctionName = (code: string, language: Language): string | null => {
    let match;
    switch (language) {
      case 'javascript':
      case 'typescript':
      ...
      case 'java':
        ...
        match = code.match(/(?:public|private|protected|static)?\s+\w+\s+([a-zA-Z0-9_]+)\s*\(/);
        return match ? match[1] : null;
      default:
        return null;
    }
  ...
  const runCode = () => {
    ...
    try {
      if (currentPuzzle) {
        // Handle puzzle execution
        if (currentLanguage === 'javascript') {
          const functionName = extractFunctionName(currentCode, currentLanguage);
          
          if (!functionName) {
            setOutput('Could not find the function to test. Make sure you have defined the function correctly.');
            return;
          }

          // Execute JavaScript puzzle and verify results
          const testResults = executeJavaScript(currentCode, functionName, currentPuzzle.tests);
          displayTestResults(testResults as TestResult[]); // Cast here
        } else {
          // For other languages, display mock execution
          mockExecuteCode(currentLanguage, currentPuzzle, displayTestResults); // Pass displayTestResults
        }
      } else {
        // Regular code execution (non-puzzle mode)
        if (currentLanguage === 'javascript') {
          // Execute JavaScript in the browser
          const output = executeJavaScript(currentCode);
          setOutput(output);
        } else {
          // Show message for other languages that require server-side execution
          setOutput(
            <div className="execution-message">
              <p>Code execution for {currentLanguage} would typically require a server-side component.</p>
              <p>In a production environment, this would send the code to a backend service that:</p>
              <ol>
              ...
              <p>For this demo, we're only displaying the code you've written.</p>
            </div>
          );
        }
      }
  ...
  // Display test results for puzzles
    ...
    const testResults = (
      <div className="test-results">
        {allPassed ? (
          ...
          <div className="test-failure">✗ Some tests failed. Check the details below.</div>
        )}
        
        <div className="test-details">
          {results.map((result, index) => (
            <div key={index} className={`test-case ${result.passed ? 'passed' : 'failed'}`}>
              <div className="test-header">Test #{index + 1}: {result.passed ? 'Passed' : 'Failed'}</div>
  ...
  // Mock execute code is now defined in codeExecution.ts and passed displayTestResults
    ...
    <div className="app-container">
      ...
      {currentPuzzle && (
        <div className="puzzle-description">
          <strong>{currentPuzzle.title}:</strong> {currentPuzzle.description}
        </div>
      )}
